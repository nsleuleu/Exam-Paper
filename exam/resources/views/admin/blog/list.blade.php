...
<body>
    ...
    <ul>
        @foreach($list_obj as $item)
            <li>
                {{$item -> name}} -
                <a href="/admin/blog/{{$item -> id}}">Show</a>&nbsp;&nbsp;
                <a href="/admin/blog/{{$item -> id}}/edit">Edit</a>&nbsp;&nbsp;
                <a href="#/admin/blog/{{$item -> id}}" id="{{$item -> id}}" class="delete-link">Delete</a>
            </li>
        @endforeach
    </ul>
        ...
        for (var i = 0; i < deleteLinks.length; i++) {
            deleteLinks[i].onclick = function () {
                ...
                if(choice){
                    ...
                    xhttp.open("DELETE", "http://localhost:8000/admin/blog/" + id, true);
